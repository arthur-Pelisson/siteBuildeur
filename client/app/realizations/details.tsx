"use client";
import RealizationDetailsLayout from "./detailsLayout";
...
const RealizationDetails = ({ slug }) => {
    ...
    } = getPostBySlug(slug.replace(/-/g, " "), "realization");
    ...
    useEffect(() => {
        ...
        if (detailsResponse !== null) {
            loadImages((detailsResponse as realizationDetails).imagesPost);
        }
    }, [page]);
    ...
    useEffect(() => {
        ...
        useEffect(() => {
          if (detailsSuccess && detailsResponse !== null) {
    ...
    console.log("detailsResponse : ", detailsResponse);
    return (
        <RealizationDetailsLayout item={detailsResponse}>
             <BtnTopPage color={"white"} />
            <div className="flex justify-center flex-wrap">
                <div className="w-[90%]  m-5 md:w-[35%] flex items-center justify-center">
                    <IframeVideo url={(detailsResponse as unknown as realizationDetails).Realization_post.video} />
                </div>
                <div className="w-[90%] p-5 m-5 md:w-[35%]">
                    <h1 className="text-2xl !text-eggs  font-bold text-center underline">
                        {getTranslation((detailsResponse as unknown as realizationDetails).translations, "title", language)}
                    </h1>
                    <p
                    className="!text-eggs !bg-black"
                        dangerouslySetInnerHTML={{
                            __html: getTranslation((detailsResponse as unknown as realizationDetails).translations, "content", language),
                        }}
                    ></p>
                     <BackBtn url={"/realizations"} style={"float-right"} color={"white"}/>
                </div>
