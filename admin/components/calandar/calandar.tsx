...
const Calendar = () => {
  ...
  return (
    <div>
      ...
      <DaysMonth daysInMonth={daysInMonth} currentDate={currentDate} daysName={weekNames.fr} today={today} posts={responsePosts} language={"fr"}/>
    </div>
  )
}
              ...
              <div className="w-[100%] h-[5rem] ">
                {event.map((post, index) => (
                  <div key={index} className="w-[100%] sm:h-[2rem] h-[3rem] bg-blue-500 border-1 overflow-hidden overflow-y-auto hide-scrollbar">
                    <Tooltip title={`${getTranslation(post.translations, "title", "fr" )}: ${dayjs(post.event_post.date_start).format("HH:mm")}`}>
                      <Link href={`/events/?slug=${post.slug.replace(/ /g, "-")} `}>
                        <p>{getTranslation(post.translations, "title", "fr" ) } : {dayjs(post.event_post.date_start).format("HH:mm")}</p>
                      </Link>
                    </Tooltip>
                  </div>
                ))} 
