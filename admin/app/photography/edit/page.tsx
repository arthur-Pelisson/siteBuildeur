...
const PhotographyPage = () => {
    ...
    const { response: content, Error: errorContent, Loading: loadingContent, Success: successContent } = getPostBySlug(slug, "photography");
    const {
    ...
    useEffect(() => {
        if (content && successContent && !loadingContent) {
            ...
            setData({
                ...
                image: (content as any).Photography_post.image,
    ...
    const setPublish = async () => {
        const waiter = await publishFetch({ url: publishParams.url + "photography/publishe/" + (content as any)?.id , method: publishParams.method });
        if (waiter) {
    ...
    const handleSubmite = async () => {
        ...
        fetchUpdatePost({ url: paramsUpost.url + "photography/" + (content as any)?.id, data: post, method: paramsUpost.method });
    };
    ...
    return (
        <div>
            ...
            <h1 className="text-center text-3xl underline">Modification d'article de photos : {data.slug}</h1>
