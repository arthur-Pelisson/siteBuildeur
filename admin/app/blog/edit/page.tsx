...
const EditeBlogPage = () => {
    ...
    const { response: content, Error: errorContent, Loading: loadingContent, Success: successContent } = getPostBySlug(slug, "blog");
    const {
    ...
    useEffect(() => {
        if (content && successContent && !loadingContent) {
            ...
            setData({
                ...
                image: (content as any).blog_post.image,
    ...
    const handleDataChange = (name: string, value: any, lg) => {
    ...
    const setPublish = async () => {
        const waiter = await publishFetch({ url: publishParams.url + "blog/publishe/" + (content as any)?.id , method: publishParams.method });
        if (waiter) {
    ...
    const handleSubmite = async () => {
        ...
        fetchUpdatePost({ url: paramsUpost.url + "blog/" + (content as any)?.id, data: post, method: paramsUpost.method });
    };
    ...
    return (
        <div>
            ...
            <h1 className="text-center text-3xl underline">Modification d'article d'images</h1>
